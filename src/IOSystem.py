...
class Index:
    """
    Manages sector, region, and impact indices within the IOSystem.

    Provides functionalities to read and update hierarchical multi-indices,
    load data from Excel files, and generate new Excel files for sectoral
    ...
    def read_configs(self) -> None:
        """
        Reads and processes multiple Excel files and loads data into corresponding
        instance variables. Validates structure and content of each Excel sheet.
        """
        try:
            ...
            # Store unit transformations for later use
            self.iosystem.index.unit_transform = self.units_df.values.tolist()

            # Create dictionary from 'general_df'
    ...
    def create_multiindices(self) -> None:
        """
        Creates MultiIndex structures for sector, region, and impact matrices.
        Generates hierarchical indices to manage relationships between
        sectors, regions, and impacts.
        """
    ...
    def update_multiindices(self) -> None:
        """
        Updates the MultiIndex structures for sector and impact matrices in the IOSystem.
        Loads the latest Excel data and creates/updates MultiIndex structures
        for key matrices.
        """
        # Load the latest config data and update multiindices
        self.read_configs()
    ...
    def write_configs(self, sheet_name: str) -> None:
        """
        Creates or updates Excel files for various datasets based on
    ...
    def update_map(self, force: bool = False) -> None:
        """
        Creates or updates a GeoDataFrame for world regions, mapping EXIOBASE regions
        to geographical regions based on the provided Natural Earth shapefile.

        Args:
            force: Whether to force update the map
        """
        try:
...
class Impact:
    """
    Class to handle impact data, including loading and
    processing impact matrices.
    """
    ...
    def load(self) -> None:
        """
        Loads impact matrices from .npy files and stores them as DataFrames.
        """
    ...
    def get_regional_impacts(self, region_indices: List[int]) -> None:
        """
        Adjusts environmental impact calculations to ensure that all sectors
        within the specified region are counted as part of the final production stage.

        Args:
            region_indices: List of region indices
        """
